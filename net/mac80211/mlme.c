					...
					const u8 *wmm_param, int len)
{
	const u8 *pos;
	size_t left;
        printk("kayas:interface name is %s\n",sdata->dev->name);
	if (len < 8)
		return false;
	...
	for (; left >= 4; left -= 4, pos += 4) {
		...
		int aci = (pos[0] >> 5) & 0x03;
printk("mac80211:kayas aifsn=%d ecwmin=%d ecwmax=%d\n",aifsn,ecwmin,ecwmax);
		if (aifsn < 2) {
			sdata_info(sdata,
...
printk("uapsd is enable for AP  = %d ssid info =%s\n",bss->uapsd_supported,assoc_data->ssid);
